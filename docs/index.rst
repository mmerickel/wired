...
Imagine an application where customers walk in the door and you want to
greet them with a Greeter. This application is simple: there's only one
Greeter.

To do this, we:

- Setup the application: make a "registry" and register some things (a
  "singleton" to hold the ``Greeter``)

- Start processing requests (greet someone) by getting stuff we need to
  process the request
